from hyperer.rg import link_matches
...
def test_multimatch():
    output = b''.join(link_matches(multimatch, b'LICENSE', b'87', 3))
    assert output.startswith(b'\x1b[0m\x1b[32m87\x1b[0m:\x1b[0m\x1b[1m')
    assert b';line=87:column=1;' in output
    assert b';line=87:column=39;' in output
    assert output.endswith(b'\x1b]8;;\x1b\\ work.\n')
...
def test_single():
    output = b''.join(link_matches(separable, b'LICENSE', b'293', 4))
    assert output.startswith(b'\x1b[0m\x1b[32m293\x1b[0m:')
...
def test_no_color():
    output = b''.join(link_matches(no_color, b'LICENSE', b'293', 4))
